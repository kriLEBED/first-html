...
function renderCarousels() {
  ...
  if (window.matchMedia("(min-width: 479px)").matches) {
    const secondSlidesIndex =
      currentSlideIdx + 1 >= slide.length ? 0 : currentSlideIdx + 1;
    slidesContainer.innerHTML += slide[secondSlidesIndex];

    if (window.matchMedia("(min-width: 767px)").matches) {
      const thirdSlidesIndex =
        secondSlidesIndex + 1 >= slide.length ? 0 : secondSlidesIndex + 1;
      slidesContainer.innerHTML += slide[thirdSlidesIndex];

      const fourthSlidesIndex =
        thirdSlidesIndex + 1 >= slide.length ? 0 : thirdSlidesIndex + 1;
      slidesContainer.innerHTML += slide[fourthSlidesIndex];

      if (window.matchMedia("(min-width: 990px)").matches) {
        const fiveSlidesIndex =
          fourthSlidesIndex + 1 >= slide.length ? 0 : fourthSlidesIndex + 1;
        slidesContainer.innerHTML += slide[fiveSlidesIndex];

        const sixSlidesIndex =
          fiveSlidesIndex + 1 >= slide.length ? 0 : fiveSlidesIndex + 1;
